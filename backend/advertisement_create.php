<?php
  ...
  if(isset($_POST['btnSave']))
  {
    ...
    if($_FILES['photo']['size'] < 1000000){
     ...
     $advertisement->set('created_by',$_SESSION['admin_id']);
     $advertisement->set('created_at',date('Y-m-d H:i:s'));
  ...
  <div class="container-fluid">
    ...
    <div class="card-body">
      ...
      <?php if(isset($status) && $status == true){ ?>
        <p class="alert alert-success text-success">Advertisement Insert Success !!</p>
      <?php } ?>
      <?php if(isset($status) && $status == false){ ?>
        <p class="alert alert-danger text-danger">Advertisement Insert Failed !!</p>
      <?php } ?>
      ...
      <form action="" method="post" id="loginForm" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input type="date" class="form-control" id="expire_date"  placeholder="Enter expire_date" name="expire_date" required="required">
        </div>
        ...
        <div class="form-group">
          ...
          <input type='file' name='photo' id='photo'  required="required">
  			...
  			<button type="submit" name="btnSave" class="btn btn-primary"><i class="fas fa-save"></i> Save Advertisement</button>
  		</form>
